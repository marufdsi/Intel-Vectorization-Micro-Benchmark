...
//  index neighbor_processing = (_deg/16)*16;
    ...
    while (1) {
//      vertex_count = 0;
        for (index i = 0; i < neighbor_processing; i += 16) {
            ...
            __m512 w_vec = _mm512_insertf32x8(_mm512_castps256_ps512(
                    _mm512_cvt_roundpd_ps(w_vec1, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC)),
                                              _mm512_cvt_roundpd_ps(w_vec2,
                                                                    _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC), 1);
            /// Mask to find u != v
            __mmask16 self_loop_mask = _mm512_cmpneq_epi32_mask(check_self_loop, v_vec);
            if ((i + 16) > neighbor_processing) {
                self_loop_mask = _mm512_kand(self_loop_mask, (all_set_mask >> (16 - neighbor_processing - i)));
            }
            ...
            __m512d affinity_vec1 = _mm512_mask_i32gather_pd(d_set0, self_loop_mask,
                                                             _mm512_extracti32x8_epi32(C_vec, 0), &pnt_affinity[0], 8);
            __m512d affinity_vec2 = _mm512_mask_i32gather_pd(d_set0, (self_loop_mask >> 8),
                                                             _mm512_extracti32x8_epi32(C_vec, 1), &pnt_affinity[0], 8);

            __m512 affinity_vec = _mm512_insertf32x8(_mm512_castps256_ps512(
                    _mm512_cvt_roundpd_ps(affinity_vec1, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC)),
                                                     _mm512_cvt_roundpd_ps(affinity_vec2, _MM_FROUND_TO_NEAREST_INT |
                                                                                          _MM_FROUND_NO_EXC), 1);
            ...
            /// It will calculate the ignorance vertices in the previous calculation.
            ...
            _mm512_storeu_pd(&ignorance_edge_weight[vertex_count],
                             _mm512_cvt_roundps_pd(_mm512_extractf32x8_ps(w_not_processed, 0), _MM_FROUND_NO_EXC));
            _mm512_storeu_pd(&ignorance_edge_weight[vertex_count + 8],
                             _mm512_cvt_roundps_pd(_mm512_extractf32x8_ps(w_not_processed, 1), _MM_FROUND_NO_EXC));
...
///                        _mm512_mask_i32scatter_ps(&pnt_affinity[0], mask, C_vec, affinity_vec, 4);
            _mm512_mask_i32scatter_pd(&pnt_affinity[0], mask, _mm512_extracti32x8_epi32(C_vec, 0),
                                      _mm512_cvt_roundps_pd(_mm512_extractf32x8_ps(affinity_vec, 0), _MM_FROUND_NO_EXC),
                                      8);
            _mm512_mask_i32scatter_pd(&pnt_affinity[0], mask >> 8, _mm512_extracti32x8_epi32(C_vec, 1),
                                      _mm512_cvt_roundps_pd(_mm512_extractf32x8_ps(affinity_vec, 1), _MM_FROUND_NO_EXC),
                                      8);*/
...
///                        __m512 affinity_vec = _mm512_i32gather_ps(C_vec, &pnt_affinity[0], 4);
            ...
            v_not_processed = _mm512_mask_compress_epi32(set0, distinct_V_mask, v_vec);
            /// Count the set bit from the mask for neighbor community
            index neigh_cnt = _mm_popcnt_u32((unsigned) distinct_C_mask);
            /// Count the set bit from the mask for ignore vertices
            index vertex_cnt = _mm_popcnt_u32((unsigned) distinct_V_mask);
            /// Store distinct neighbor community
            _mm512_storeu_si512(&pnt_neigh_comm[neigh_counter], distinct_comm);
            /// Store ignore vertices
            _mm512_storeu_si512(&ignorance_vertex[vertex_count], v_not_processed);
...
///                        _mm512_mask_i32scatter_ps(&pnt_affinity[0], mask, C_vec, affinity_vec, 4);
        ...
        neighbor_processing = vertex_count;
